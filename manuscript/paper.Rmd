...
newdataEP <-
  with(dat, data.frame(PIM.GP_numeric = c(0.25,.5,3),
...
ggdat <- data.frame(PIM.GP_numeric = c(rep(c(.5,3),each = nnn)),
  surv = c(
...
```{r fig:estsurv, echo = FALSE, fig.cap = "Estimated Survival curve for EF for defenders.  All other variables are fixed at their mean level (GP = 285.8, Ht = 72.7in, Wt = 197.03, and Year of birth 1974.5."}
coxphmod_Total.Fights <- coxph(Surv ~ Total.Fights + northamerica + GP.x + Ht + Wt + DOB.Year + Pos2, data = dat)
# summary(coxphmod_EP)
# library(xtable)
# xtable(summary(coxphmod_EP)$coef[,-4],digits = 5)
# cox.zph(coxphmod_EP)
...
newdataEF <-
  with(dat, data.frame(Total.Fights = c(0,1,7),
                       GP.x = rep(mean(GP.x), 3),
                       Ht = rep(mean(Ht), 3),
                       Wt = rep(mean(Wt), 3),
                       DOB.Year = rep(mean(DOB.Year), 3),
