---
title: |
  |  \large Enforcers are not okay, probably.  
author: | 
abstract: |
...
@Silverwood2015 looked anecdotally at how enforcers fit into a team atmosphere noticed that teammates put their enforcers on a pedestal. Performing well as an enforcer leads to the player being seen as a protector. When an enforcer fulfills these additional responsibilities to being on the ice as a normal player, players say that it is common for that enforcer to receive perks, such as not paying for their own meals or getting the best seat on the bus. They go as far as to say these players are "deified", emphasizing the social aspects of hockey that surround this history of violence. 

While any player on a hockey team can get into a fight, enforcers are exposed to many more fights than the typical NHL player. As a result of being exposed to additional contact, they are potentially exposed to additional head trauma beyond the amount associated with playing hockey. There are many questions about the long-term implications of this head trauma and its potential to result in negative health outcomes in the future. The ingrained nature of fighting into the game of hockey is greatly criticized and there are many active debates about its current declining trend. Figure \ref{fig:enforcer_density} shows a decline in the proportion of enforcers to non-enforcers as players get younger. The question is, should the game continue to move away from contact-driven strategy in favor of younger, faster, highly-skilled players?
...
Recently, @Popkin2023 explored the effects of being an enforcer on mortality rates, ages at death, and causes of death to address the long-term impact this repeated contact might have. They defined enforcers in two ways: (1) those who had 50 or more career fights (E-F cohort) and (2) those who had, on average, more than 3 penalty minutes per game (E-P cohort). 

Based on their analysis (a matched cohort study) they find that there is no statistically significant difference in the mortality rates between enforcers and controls for either definition of enforcer, but they do conclude that enforcers died on average about 10 years earlier than their non-enforcer counterparts. In addition, they draw the conclusion that there is not a significant difference in all-cause mortality rate but find enforcers died more frequently of drug overdose and suicide. 

However, we believe that some of the methods used in @Popkin2023 to reach these conclusions are not statistically appropriate.  Most notably, the method used to reached their most sensational finding - enforcers died on average about 10 years younger than their non-enforcer counterparts - is statistically unsuitable. Specifically, to reach this conclusion, they use the data from the matched cohort study but then condition on death.  This effectively breaks the matching that they appropriately used to compare mortality rates, and therefore this conclusion is not statistically valid. 


Here, we attempt not to replicate the results of the analysis in @Popkin2023, but what we believe to be more appropriate statistical analysis, specifically survival analysis methodology. This shift in perspective allows us to overcome the weakness of mortality rate, which is that it is a function of when it is calculated (i.e. In all cases, mortality rate will be 100% when all members of the original sample have died). Instead, survival analysis provides a survival curve, which can be used to analyze how the probability of survival changes over time.  In addition, we augmented the data used in @Popkin2023 to increase the sample size in the analysis and include more deceased players in the data set. 

As mentioned earlier, the outcome of this study is relevant to the debates about regulations and discourse around head trauma in hockey. An accurate analysis is imperative to prevent the spread of misinformed conclusions and best inform regulatory safety practices going forward.

... talk about our conclusions
...
# Data 
## Data from orignal paper
We obtained the two data sets used in @Popkin2023: general table (n = 7,432) and death table (n = 45).  As in @Popkin2023, we considered only players who's first season began in 1967 or later and played any games between the 1967-68 and the 2021-22 NHL seasons.  This leaves 6,039 players after filtering (the exact number mentioned in @Popkin2023). We then removed 4 players with missing height, leaving us with `r nrow(dat)`  remaining observations, for our re-analysis. 
...
## Additional Data
While @Popkin2023 only looks at a total of 45 deceased players  (21 enforcers and 24 controls), we identified `r sum(dat$dead)` players who were included in the data used in @Popkin2023 who were deceased as of May 7, 2023 based on data from hockey-reference.com. Of these additional `r sum(dat$dead) - nrow(death_JAMA)` observations of death, 2 fall in the E-F cohort, and 4 fall in the E-P cohort.
...
Of the players included in the analysis, on average, enforcers in this data set tended to be older than the non-enforcers (EF: `r summ$mnage[summ$EF]` vs non-EF: `r summ$mnage[!summ$EF]` and EP: `r summEP$mnage[summEP$EP]` vs non-EP: `r summEP$mnage[!summEP$EP]`).  Additionally, as seen in Tables \ref{tab:EDAEF} and \ref{tab:EDAEP} enforcers tended to also be taller and heavier on average than non-enforcers. The means of age, weight, height and goals are all significantly different between the enforcers and non-enforcer groups for both definitions of enforcers.  
...
Figures \ref{fig:KMEF} and \ref{fig:KMEP} show Kaplan-Meier plots for EF and EP.  There is substantial overlap in the K-M curves between the enforcer and non-enforcers groups when enforcers is defined using the EF criteria. However, the enforcer group when defined by the EP criteria exhibits evidence of lower survival probabilities starting around age 50 relative to the non-enforcer group, though it is important to remember that these two groups differ substantially in age, height, and weight.

In order to account for these differences betwen the groups, Cox proportional hazards models @CoxPH were fit: one for each definition of enforcer defined in @Popkin2023 (i.e. E-F and E-P) with time until death as the outcome variable.  In addition, to indicators for each definition of enforcer, games played (GP), height (Ht), weight (Wt), year of birth (DOB), and position indicators (Center (C), Wing (W), or Defender (D)) were added to each model as control variables.  
...
# Results {#sec:results}
Table \ref{tab:EF} shows the results for the Cox proportional hazards model when enforcers were defined based on their number of career fights.  No statistically significant difference was observed in the hazard functions of enforcers versus non-enforcers (p-value = 0.549). However, a significant difference is observed between enforcers and non-enforcers when the enforcer is defined based on their number of penalty minutes per game (p-value = 0.00169).  The estimated hazard ratio for E-P is 2.29, indicating a 129\% increase in the hazard for enforcers vs non-enforcers when all other variables are held constant.  

Figure \ref{fig:estsurv} shows the estimated survival curves for enforcers (E-P) versus non-enforcers for defenders with all continuous variables held constant at mean levels.  Using this, the estimated survival rate at the age of 50 for enforcers vs non-enforcers is 96.57\% (95\% CI: 94.63\%, 98.56\%) vs 98.49\% (95\% CI: 97.98\%, 99.00\%).  For the same conditions at the age of 75, the estimated survival rate for enforcers is about 74.4\% (95\% CI: 61.4\%, 90.32\%) compared to non-enforcers estimate survival rate of 87.95\% (95\% CI: 83.09\%, 93.10\%).  The estimated survival rate for enforcers drops an additional `r (96.57-74.4)-(98.49 - 87.95)`\% between the ages of 50 and 75 versus non-enforcers.

These results suggest that the conclusions of @Popkin2023 are overstated due to the differences in exposure time between the groups at the time of their final analysis.




join all from jama nhl data, condition on death from jama deaths and look at yob between enforcer/control
```{r}
...
jama_short %>%
  ...
  group_by(enforcer) %>%
  summarize(mean(DOB.Year))


jama_short %>%
  mutate(deathage = as.numeric(difftime(Death.Date,
                                        Birth.Date,
                                        units = "weeks")/52.25)) %>%
  filter(Player %in% jama_deaths$Enforcers) %>%
  mutate(enforcer = ifelse(EF == 1 | EP == 1, 1, 0)) %>% 
  group_by(enforcer) %>%
  summarize(mean(deathage))
