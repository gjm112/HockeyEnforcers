...
```{r enforcer_density, echo = FALSE, fig.cap='Proportion of Enforcer-Status Players to All Players by Date of Birth'}
...
cohort_dat$cohort <- factor(cohort_dat$cohort, levels = c('All Players', 'Enforcer'))
...
\end{tabular}
\end{center}
\caption{Descriptive Summary of All Hockey Players who Started after 1967 and Played at Least One Game Between the 1967-68 and 2022-2023 NHL seasons
...
\hline
 Variable & Enforcers & Non-Enforcers & p-value \\
 \hline
...
\end{tabular}
\end{center}
\caption{Comparison of Enforcers to Non-Enforcers when Enforcer is Defined by Career Fights (E-F) \label{tab:EDAEF}}
\end{table}
...
\caption{Comparison of Enforcers to Non-Enforcers when Enforcer is Defined by Penalty Minutes (E-P) \label{tab:EDAEP}}
\end{table}
...
```{r KMEF, echo = FALSE, fig.cap="Kaplan Meier Plot Comparing Survival Rates of Enforcers to Non-Enforcers when Enforcer is Defined by Career Fights (E-F)" }
library(ggsurvfit)
...
```{r KMEP, echo = FALSE, fig.cap="Kaplan Meier Plot Comparing Survival Rates of Enforcers to Non-Enforcers when Enforcer is Defined by Penalty Minutes (E-P)" }
library(ggsurvfit)
...
# Results {#sec:results}
Table \ref{tab:EF} shows the results for the Cox proportional hazards model when enforcers were defined based on their number of career fights.  No statistically significant difference was observed in the hazard functions of enforcers versus non-enforcers (p-value = 0.490). However, a significant difference is observed between enforcers and non-enforcers when the enforcer is defined based on their number of penalty minutes per game (p-value = 0.00125).  The estimated hazard ratio for E-P is 2.35, indicating a 135\% increase in the hazard for enforcers vs non-enforcers when all other variables are held constant.  
...
\begin{tabular}{|c|c|c|c|c|}
  \hline
Covariate  & $\beta$ & $e^{\beta}$ & s.e. & p-value \\ 
  \hline
  $I_{\text{E-F}}$ & 0.200 & 1.221 & 0.289 & 0.490 \\ 
  GP & -0.00061 & 0.999 & 0.00023 & 0.00675 \\ 
  Ht & -0.0341 & 0.966 & 0.045 & 0.445 \\ 
  Wt  & 0.0213 & 1.0216 & 0.00658 & 0.00118 \\ 
  DOB & -0.0169 & 0.983 & 0.00825 & 0.0405 \\ 
  $I_{\text{Defense}}$ & -0.384 & 0.681 & 0.175 & 0.028 \\ 
  $I_{\text{Wing}}$ & -0.284 & 0.753 & 0.160 & 0.075 \\ 
   \hline
\end{tabular}
\caption{Cox Proportional Hazards Model Output Comparing Enforcer to Non-Enforcer when Enforcer is Defined by Career Fights (E-F) and Controlling for Games
Played (GP), Height (Ht), Weight (Wt), Year of Birth (DOB), and Position Indicators \label{tab:EF}}
\end{table}
...
\begin{tabular}{|c|c|c|c|c|}
 ...
 Covariate & $\beta$ & $e^{\beta}$ & s.e. & p-value \\ 
  \hline
$I_{\text{E-P}}$ & 0.854 & 2.350 & 0.265 & 0.00125 \\ 
  GP & -0.00052 & 0.999 & 0.00022 & 0.0159 \\ 
  Ht & -0.0278 & 0.973 & 0.0444 & 0.531 \\ 
  Wt & 0.0186 & 1.0188 & 0.00659 & 0.00480 \\ 
  DOB & -0.0170 & 0.983 & 0.00831 & 0.0410 \\ 
  $I_{\text{Defense}}$ & -0.388 & 0.678 & 0.175 & 0.0269 \\ 
  $I_{\text{Wing}}$ & -0.313 & 0.731 & 0.160 & 0.0509 \\ 
   \hline
\end{tabular}
\caption{Cox Proportional Hazards Model Output Comparing Enforcer to Non-Enforcer when Enforcer is Defined by Penalty Minutes (E-P) and Controlling for Games Played (GP), Height (Ht), Weight (Wt), Year of Birth (DOB), and Position Indicators \label{tab:EP}}
\end{table}
...
ggplot(aes(x = time,y = surv, color = enforcer), data = ggdat) + geom_path() + geom_path(aes(x = time,y = lower, color = enforcer), lty = 3) + geom_path(aes(x = time,y = upper, color = enforcer), lty = 3) + ylab("Overall Survival Probability") + labs(color = NULL)
...
Finally, while our finding that there is an association between increased hazard and enforcers defined by penalty minutes, we note that this is a rather arbitrary definition of enforcer.  We hypothesize that the effect is not actually driven by the "enforcer" category, but simple by an increase in penalty minutes.  We posit that a higher average number of penalty minutes is associated with more general contact, including but not limited to fighting.  Further, these contacts in hockey are often at high speeds due the the nature of the game (i.e. the players are on skates).  We leave this proposal as future work, but even without exploring this avenue, we believe this work adds to the body of literature relating contact in sports and a reduction in life spans.  
...
We would like to thank the authors of @Popkin2023 for their generosity and willingness to share the data provided for the above analysis.
