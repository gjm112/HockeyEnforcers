...
Wed Sep  4 18:44:57 2024-->

\begin{table}[ht]
\centering
\begin{tabular}{rrrrr}
  \hline
 & enforcer & mean\_YOB & mean\_age\_at\_death \\ 
  \hline
1 & 0.00 & 1958.29 & 56.46 \\ 
  2 & 1.00 & 1962.81 & 47.15 \\ 
   \hline
\end{tabular}
\caption{Original Paper Conclusions Data Summary
 \label{tab:popkin_conclusions}}
\end{table}



These results suggest that the conclusions of @Popkin2023 are overstated. We believe this may be due to the differences in exposure time between the groups at the time of their concluding analysis. Table \ref{tab:popkin_conclusions} shows that by conditioning on death, the mean year of birth becomes significantly different between enforcer vs non-enforcer groups. Before conditioning on death, the mean year of birth for each group was `r round(precnclsn_sum[[1,2]], 2)` for non-enforcers and `r round(precnclsn_sum[[2,2]], 2)` for enforcers. Conditioning on death contradicts any previous equality of means due to matching, resulting in the enforcer group having an average year of birth much later than the control group, and thereby it is no longer appropriate to compare their mortality rate BECAUSEEEE.....
